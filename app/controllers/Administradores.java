...
public class Administradores extends Controller{
	...
	//buscar atendente
	if (buscaAtd != null) {
		atendentes = Atendente.find("lower (nome) like ?1 " + 
	"or lower(email) like ?1", "%" + buscaAtd.toLowerCase() + "%").fetch();
	}
	...
	if (buscaPacientes != null) {
		pacientes = Paciente.find("lower (nome) like ?1 " + 
	"or cpf like ?1", "%" + buscaPacientes.toLowerCase() + "%").fetch();
	}
